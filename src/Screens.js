  ...
  handleInput(inputType, inputData) {
    if (inputType === "keydown") {
      if (
        ...
        inputData.keyCode === ROT.KEYS.VK_H
      ) {
        this.move(-1, 0);
      } else if (
        inputData.keyCode === ROT.KEYS.VK_RIGHT ||
        inputData.keyCode === ROT.KEYS.VK_L
      ) {
        this.move(1, 0);
      } else if (
        inputData.keyCode === ROT.KEYS.VK_UP ||
        inputData.keyCode === ROT.KEYS.VK_K
      ) {
        this.move(0, -1);
      } else if (
        inputData.keyCode === ROT.KEYS.VK_DOWN ||
        inputData.keyCode === ROT.KEYS.VK_J
      ) {
        this.move(0, 1);
      } else if (inputData.keyCode === ROT.KEYS.VK_Y) {
        // LEFT-UP
        this.move(-1, -1);
      } else if (inputData.keyCode === ROT.KEYS.VK_U) {
        // RIGHT-UP
        this.move(1, -1);
      } else if (inputData.keyCode === ROT.KEYS.VK_B) {
        // LEFT-DOWN
        this.move(-1, 1);
      } else if (inputData.keyCode === ROT.KEYS.VK_N) {
        // RIGHT-DOWN
        this.move(1, 1);
      }
    }
...
class winScreen extends Screen {
  ...
  render(display) {
    display.drawText(2, 1, "%b{green}You win! :)");
  }
  handleInput(inputType, inputData) {}
}
